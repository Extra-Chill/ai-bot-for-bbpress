...
function ai_bot_register_all_settings() {
    ...
    register_setting( $settings_group, 'ai_bot_enable_local_search', 'absint' );
    ...
    // Add Fields
    // AI Provider Configuration Field (WordPress core AI Credentials)
    add_settings_field(
    ...
    // Enable Local Search Field
    add_settings_field(
        'ai_bot_enable_local_search', // ID
        __( 'Enable Local Search', 'ai-bot-for-bbpress' ), // Title
        'ai_bot_enable_local_search_callback', // Callback
        $page_slug, // Page
...
function ai_bot_api_settings_section_callback() {
    echo '<p>' . esc_html__( 'Configure your AI provider API keys in Settings → AI Credentials, then configure the bot user here.', 'ai-bot-for-bbpress' ) . '</p>';
}
...
function ai_bot_context_settings_section_callback() {
    echo '<p>' . esc_html__( 'Configure how the bot accesses information to provide contextually relevant answers.', 'ai-bot-for-bbpress' ) . '</p>';
}

// Field Callbacks
function ai_bot_provider_config_callback() {
    $url = admin_url( 'options-general.php?page=wp-ai-client-api-credentials' );

    printf(
        '<p><a href="%s">%s</a></p>',
        esc_url( $url ),
        esc_html__( 'Open AI Credentials settings', 'ai-bot-for-bbpress' )
    );

    echo '<p class="description">' . esc_html__( 'API keys are configured in WordPress core under Settings → AI Credentials.', 'ai-bot-for-bbpress' ) . '</p>';
}
...
function ai_bot_enable_local_search_callback() {
    $enabled = (int) get_option( 'ai_bot_enable_local_search', 1 );

    echo '<input type="hidden" name="ai_bot_enable_local_search" value="0" />';

    echo '<label>';
    echo '<input type="checkbox" name="ai_bot_enable_local_search" value="1" ' . checked( 1, $enabled, false ) . ' /> ';
    echo esc_html__( 'Allow the bot to use the built-in local search ability.', 'ai-bot-for-bbpress' );
    echo '</label>';

    echo '<p class="description">' . esc_html__( 'If disabled, no built-in abilities are enabled by default. You can still enable other abilities using the ai_bot_enabled_abilities filter.', 'ai-bot-for-bbpress' ) . '</p>';
}
